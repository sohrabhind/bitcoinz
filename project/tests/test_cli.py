...
from project.bitcoinz import config
from project import cli
from project.bitcoinz.exceptions import DepositAddressDoesntExistException


@pytest.fixture
def response():
    import requests
    return requests.get('https://bitcoinz.gemini.com/')
...
def test_cli_basic():
    ...
    assert 'Welcome to the BitcoinZ network' in result.output
...
def test_cli_creates_address():
    ...
    output_re = re.compile(
        r'You may now send BitcoinZs to address [0-9a-zA-Z]{32}. '
